...
from unit_tests.rest_qa_api_tests.tests_utils import DummyApiValidationConfig, DummyConfigBuilder, DummyResponseBuilder
...
class TestRunCheckersBranches:
   def test_BaseRESTCheckers_instances_for_customer_checker(self, _, response, builder):
        JSONCheckers.status = 400
        ...
        assert f"Invalid status code. Expected '{JSONCheckers.status}', " \
...
class TestCustomLogger:
    def test_logging_after_successful_execution_of_customer_checker(self, _, response, builder, caplog):
    ...
    def test_function_call(self, _, response, builder):
        response.status_code = 200
        check_status_code = 400
        builder.endpoint.response_model.custom_checkers.append(check_status(check_status_code))
        with pytest.raises(RestResponseValidationError) as excinfo:
