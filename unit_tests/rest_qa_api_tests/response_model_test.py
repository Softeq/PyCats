...
from common._rest_qa_api.rest_utils import pycats_dataclass
...
def test_no_method_impl(field_name):
    with pytest.raises(TypeError) as excinfo:
        ...
        test_response_class = pycats_dataclass(type("_ResponseModel", (BaseResponseModel,),
...
def test_missing_decorator_exception():
    with pytest.raises(MissingDecoratorError) as excinfo:
        ...
        test_response_class(config)
    assert "Child dataclass _ResponseModel should have @pycats_dataclass decorator" in str(excinfo.value)


def test_repr_format():
    test_response_obj = pycats_dataclass(type("_ResponseModel", (BaseResponseModel,),
